...
import { ArrowLeft, Save, User, Building, Phone, Mail, MapPin } from 'lucide-react';
...
const CreateClient = () => {
  ...
  const [formData, setFormData] = useState({
    ICLTPRO: 0, // 0 = Particulier, 1 = Professionnel
    RAISON_SOCIALE: '',
    NOMFAMILLE: '',
    PRENOM: '',
    ADRESSE: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate(`/clients/${newContact.IDCONTACT}`);
      } else {
        const error = await response.text();
        alert(`Erreur lors de la création du contact: ${error}`);
      }
    } catch (error) {
